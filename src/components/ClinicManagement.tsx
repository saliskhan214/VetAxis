...
export function ClinicManagement({ 
  ...
  const handleCreateSoap = (e: FormEvent) => {
    e.preventDefault();
    if (!newSoap.patientName || !newSoap.ownerName) return;

    setConfirmState({
      title: "Lock and Publish SOAP Note",
      message: `Are you sure you want to lock and publish this SOAP clinical record for ${newSoap.patientName}? This action is irreversible and ensures professional digital veterinary registry integrity.`,
      confirmText: "Publish & Lock",
      isDanger: true,
      onConfirm: async () => {
        try {
    ...
    <div id="clinic-management-desk" className="space-y-6 max-w-7xl mx-auto w-[95%] text-left pb-16">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-[#e3dec9] pb-3">
        {[
          ...
          { id: 'prescriptions', label: 'Medications Writer', icon: Pill },
        ].map(tab => {
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between border-b border-[#e3dec9] pb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => triggerPrintDoc('soap', activeSoap.id)}
                        className="cursor-pointer bg-[#5a5a40] hover:bg-[#3e3e2b] text-white py-1.5 px-3 rounded-xl text-xs font-bold border-none flex items-center gap-1.5 shadow-sm transition-all"
                      >
                  ...
                  {/* Physical Digital Paper Sheet Container */}
      ...
      {/* ─── TAB 4: PRESCRIPTIONS & MEDICATIONS WRITER ───────────────── */}
      {activeTab === 'prescriptions' && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="font-serif font-black text-lg text-[#5a5a40]">Dosage Calculators & Medication logs</h2>
              <p className="text-[10px] text-gray-500 font-mono">SEARCH DRUG STOCK WITH CONTRAINDICATION INTEGRITY VERIFIER</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-6 md:col-span-1">
              {/* Prescription Dispatch Registry Card List */}
              <div className="bg-white border border-[#e3dec9] border-b-[4px] border-b-[#cdc6ad] rounded-2xl p-4 space-y-3 max-h-[50vh] overflow-y-auto">
                <h3 className="font-serif font-bold text-sm text-[#5a5a40] mb-2 border-b border-[#f4f1e9] pb-2">Prescription Dispatch Registry</h3>
                {prescriptions.length === 0 ? (
                  <p className="text-stone-400 text-xs text-center py-6">No prescribed cycles logged yet.</p>
                ) : (
                      ...
                      <div
                        ...
                        className={`p-3 rounded-xl border transition-all cursor-pointer text-left ${
                          activePrescription?.id === p.id 
                            ? 'bg-amber-50/25 border-[#a0522d]' 
                            : 'bg-stone-50 hover:bg-[#fdfbf7] border-gray-200'
                        }`}
                      >
                        <div className="flex justify-between items-center">
                          <span className="font-mono text-[9px] text-[#a0522d] font-bold">{p.id}</span>
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeletePrescription(p.id, p.patientName);
                              }}
                              className="cursor-pointer p-1 text-gray-400 hover:text-red-500 rounded hover:bg-red-50 transition-all border-none"
                        ...
                        <span className="block font-bold text-xs mt-1 text-gray-900">{p.patientName}</span>
                        <span className="block text-[10px] text-gray-600">Owner: {p.ownerName}</span>
                        {p.isControlled && (
                          <span className="text-[8px] uppercase font-bold text-rose-700 font-mono mt-0.5 block">⚠️ CONTROLLED DRUG</span>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            {/* Read right panel - Beautifully configured Digital Rx Prescription Pad Paper */}
            <div className="bg-white border border-[#e3dec9] border-b-[4px] border-b-[#cdc6ad] rounded-2xl p-4 md:col-span-2 space-y-6">
              {activePrescription ? (
                <div className="space-y-4">
                  {/* Action items above the paper */}
                  <div className="flex items-center justify-between border-b border-[#e3dec9] pb-3">
                    <span className="font-mono text-[9px] text-blue-900 font-bold bg-blue-100/60 px-2 py-0.5 rounded-md">REGISTERED CLINICAL DISPATCH SHEET</span>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleDeletePrescription(activePrescription.id, activePrescription.patientName)}
                        className="cursor-pointer bg-rose-50 hover:bg-rose-100 text-rose-700 py-1.5 px-3 rounded-xl text-xs font-bold border border-rose-200 flex items-center gap-1.5 transition-all"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                        Delete
                      ...
                      <button
                        onClick={() => triggerPrintDoc('presc', activePrescription.id)}
                        className="cursor-pointer bg-[#5a5a40] hover:bg-[#3e3e2b] text-white py-1.5 px-3 rounded-xl text-xs font-bold border-none flex items-center gap-1.5 shadow-sm transition-all"
                      >
                        <Printer className="w-3.5 h-3.5" />
                        Print Rx Sheet
                  ...
                  {/* Physical Digital Rx Paper Pad Sheet Container */}
                  <div className="relative bg-[#fdfcf7] border-2 border-[#e3dec9] rounded-xl p-8 shadow-md overflow-hidden font-serif text-[#3e3d36] min-h-[500px]">
                    {/* Big Watermark ℞ */}
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.04] select-none">
                      <span className="font-serif text-[380px] font-black text-rose-800">℞</span>
                    </div>

                    {/* Red clinical pad left margin line */}
                    <div className="absolute left-6 top-0 bottom-0 w-[1.5px] bg-red-400/40 pointer-events-none" />

                    {/* Official Medical Letterhead Block */}
                    <div className="border-b-2 border-stone-800 pb-4 mb-6 ml-6 text-center relative">
                      <h2 className="text-stone-900 font-black text-xl font-serif uppercase tracking-tight">{user.name || 'VetAxis Official Clinical Center'}</h2>
                    ...
                    {/* Metadata block in typewriter aesthetics */}
                    <div className="ml-6 mb-6 bg-stone-50 border border-stone-200 rounded-lg p-3 grid grid-cols-2 gap-3 text-[11px] font-mono shadow-inner">
                      <div>
                        <span className="text-gray-500 block uppercase text-[8px] font-bold">Patient Name:</span>
                        <span className="font-bold text-stone-900">{activePrescription.patientName}</span>
                      </div>
                      <div>
                        <span className="text-gray-500 block uppercase text-[8px] font-bold">Patient Weight:</span>
                        <span className="font-bold text-stone-900">{activePrescription.weightKg} kg</span>
                      </div>
                      <div>
                        <span className="text-gray-500 block uppercase text-[8px] font-bold">Owner Registrar:</span>
                        <span className="font-bold text-stone-900">{activePrescription.ownerName}</span>
                      </div>
                      <div>
                        <span className="text-gray-500 block uppercase text-[8px] font-bold">Registrar Contact:</span>
                        <span className="font-bold text-stone-900">{activePrescription.ownerPhone || 'No contact'}</span>
                      </div>
                      <div>
                        <span className="text-gray-500 block uppercase text-[8px] font-bold">Date Issued:</span>
                        <span className="font-bold text-stone-900">{activePrescription.date}</span>
                      </div>
                      <div>
                        <span className="text-gray-500 block uppercase text-[8px] font-bold">Prescription ID:</span>
                        <span className="font-bold text-rose-800">{activePrescription.id}</span>
                      </div>
                    </div>

                    {/* Big stylized ℞ watermark before listing drugs */}
                    <div className="ml-6 mb-4 flex items-center gap-1 border-b border-stone-200 pb-2">
                      <span className="text-3xl font-bold font-serif text-rose-800">℞</span>
                      <span className="font-mono text-[9px] font-bold text-stone-500 uppercase tracking-wider mt-2">Medication Dispensing Orders</span>
                    </div>

                    {/* Detailed List of Prescribed Medicines */}
                    <div className="ml-6 space-y-4">
                      {activePrescription.drugs.map((d, index) => (
                        <div key={index} className="border-b border-[#f4f1e9] pb-3 space-y-1 text-left">
                          <div className="flex justify-between items-center">
                            <span className="font-bold text-[13px] text-stone-900 font-serif">
                              {index + 1}. {d.name} {d.brandName ? `(${d.brandName})` : ''}
                            </span>
                            <span className="text-[8px] px-1.5 py-0.5 rounded bg-gray-100 text-stone-600 font-mono font-bold">
                              {d.isGeneric ? 'Generic Base' : 'Brand Specific'}
                            </span>
                          </div>
                          
                          <div className="grid grid-cols-3 gap-2 text-[11px] font-mono text-stone-700 pt-0.5">
                            <div className="col-span-1">
                              <span className="text-stone-400 text-[8px] font-bold block">DOSAGE:</span>
                              <span className="text-stone-800 font-serif">{d.dosage}</span>
                            </div>
                            <div className="col-span-2 pl-2 border-l border-stone-100">
                              <span className="text-stone-400 text-[8px] font-bold block">INREDIENT & SCHEDULE INSTRUCTIONS:</span>
                              <span className="italic text-stone-800 font-serif font-semibold">{d.instructions}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                    ...
                    {/* Footer stamps / credentials */}
                    <div className="ml-6 mt-8 pt-6 border-t border-stone-300 flex justify-between items-end">
                      <div className="text-left">
                        <span className="block font-mono text-[8px] text-stone-400 uppercase">Electronically Signed Validation</span>
                        <div className="text-emerald-800 font-serif italic text-xs font-bold mt-1 bg-emerald-50 border border-emerald-200 rounded px-2.5 py-1 inline-block">
                          🖋️ {activePrescription.vetSignature}
                        </div>
                      </div>

                      <div className="text-right">
                        {activePrescription.isControlled ? (
                          <div className="border-2 border-red-600 rounded-lg p-1.5 text-center transform rotate-[-2deg] inline-block shadow-xs bg-red-50/15">
                            <span className="block text-[8px] font-mono uppercase font-black tracking-tight text-red-600">⚠️ CONTROLLED DRUG</span>
                            <span className="block text-[6.5px] font-mono text-red-600 font-bold">{activePrescription.controlledLogRef}</span>
                          </div>
                        ) : (
                          <span className="px-2 py-1 border border-emerald-300 bg-emerald-50 text-emerald-800 text-[8px] font-bold font-mono rounded-md inline-block">
                            ✓ VERIFIED DISPENSED
                          </span>
                        )}
                      </div>
                ...
                <div className="py-24 text-center text-gray-400 space-y-2">
                  <Pill className="w-12 h-12 mx-auto opacity-30 text-[#5a5a40]" />
                  <p className="font-bold text-xs">Select a prescription from the Registry</p>
                  <p className="text-[10px]">Select any registered/issued prescription details on the left list to review detailed formulas, compliance registry reports, and printed sheets in digital Rx paper format.</p>
                </div>
              )}
            </div>
          </div>
      ...
      <AnimatePresence>
        {showSoapModal && (
          <div className="fixed inset-0 bg-black/45 backdrop-blur-xs z-[500] flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center border-b border-[#e3dec9] pb-3">
                <h3 className="font-serif text-lg font-black text-[#5a5a40]">Write SOAP Clinical Record</h3>
              ...
              <form onSubmit={handleCreateSoap} className="space-y-4 text-xs">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input type="text" placeholder="e.g. Sheru (Shepherd)" required value={newSoap.patientName} onChange={(e) => setNewSoap({...newSoap, patientName: e.target.value})} className="w-full bg-white border border-[#e3dec9] p-2 rounded-xl" />
                  </div>
                  <div className="space-y-1">
                    <label className="font-bold text-gray-700">Owner Full Name</label>
                ...
                <div className="space-y-1">
                  <label className="font-bold text-emerald-800">O - Objective Examination (Vitals, Temperature, HR, physical palpation findings)</label>
                  <textarea required placeholder="Write clinical examinations/vitals..." value={newSoap.objective} onChange={(e) => setNewSoap({...newSoap, objective: e.target.value})} className="w-full bg-white border border-[#e3dec9] p-2 rounded-xl h-20" />
                </div>
                ...
                <div className="space-y-1">
                  <label className="font-bold text-blue-900">P - Clinical Plan (Procedures, drugs, follow-up timelines)</label>
                ...
                <div className="flex justify-end gap-2 pt-3 border-t">
                  ...
                  <button type="submit" className="cursor-pointer bg-[#5a5a40] hover:bg-[#3e3e2b] text-white px-5 py-2 rounded-xl font-bold border-none">Lock Soap Card</button>
                </div>
              </form>
            </motion.div>
          </div>
