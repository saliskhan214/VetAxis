...
import { UserProfile, VetNotification, isGuestUser } from '../types';
...
export function Navbar({ 
  ...
  const isClinicSubscribed = user ? (user.subscriptionTier === 'Silver' || user.subscriptionTier === 'Gold' || user.subscriptionTier === 'Platinum') : false;
  ...
  const initials = (user?.name || 'Visitor')
    .trim()
    .split(/\s+/)
    .map(w => w[0] || '')
    .slice(0, 2)
  ...
  return (
    <>
      <nav className="sticky top-0 z-[200] max-w-7xl mx-auto w-[95%] mt-4 rounded-2xl border border-[#e3dec9] border-b-[4px] border-b-[#cdc6ad] bg-white/90 backdrop-blur-md px-4 md:px-8 h-18 flex items-center justify-between shadow-[0_10px_30px_-10px_rgba(90,90,64,0.15)]">
        ...
        {/* Right Header Navigation - Visitor Login or Member Actions + Menu Button */}
        <div className="flex items-center gap-2 sm:gap-3">
          {isGuest ? (
            /* Visitor / Guest Header Actions */
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              onClick={() => {
                if (onRequireAuth) onRequireAuth("Please log in or sign up to access your verified clinical profile, message clinics, and apply for jobs.");
              }}
              className="flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2 rounded-xl bg-[#5a5a40] hover:bg-[#3e3e2b] text-white text-xs font-black border-b-[3px] border-b-[#2e2e1f] shadow-xs cursor-pointer transition-all"
      ...
      {/* Three Lines / Hamburger Menu Trigger Button */}
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onMouseEnter={() => {
            ...
            className={`relative flex items-center justify-center p-2.5 rounded-xl border border-b-[3px] transition-all duration-150 cursor-pointer shadow-xs ${
              isSidebarOpen 
                ? 'bg-[#5a5a40] text-white border-[#5a5a40] border-b-[#3e3e2b]' 
                : 'bg-white text-[#5a5a40] border-[#e3dec9] border-b-[#cdc6ad] hover:bg-[#fcf9f2] hover:border-[#5a5a40]'
            }`}
            ...
            >
              ...
              <div>
                <div className="p-5 flex items-center justify-between border-b border-[#f4f1e9]">
                  <div className="flex items-center gap-2 font-serif text-xl font-bold text-[#5a5a40]">
                    ...
                    {user?.subscriptionTier ? (
                      ...
                      <span className="bg-[#f4efe4] text-[#5a5a40] border border-[#e3dec9] rounded-md text-[8px] font-extrabold px-1.5 py-0.5 tracking-wider uppercase">
                        Visitor
                      </span>
                    ) : null}
                  </div>
                ...
                <div className="p-4 flex flex-col gap-2">
                  <p className="px-3 text-[10px] uppercase font-bold text-[#a49f92] tracking-wider mb-2">
                    {isGuest ? 'Public Network & Tools' : 'Systems Panel'}
                  </p>
                  
                  {navItems
                    .filter(item => {
                      if (item.id === 'admin') {
                        return !isGuest && (user?.email?.toLowerCase() === 'vetaxis360@gmail.com' || user?.email === 'saliskhan214@gmail.com' || user?.isAdmin === true);
                      }
                      if (item.id === 'subscription') {
                        return !isGuest && (user?.role === 'clinic' || user?.role === 'doctor');
                      }
                      if (item.id === 'clinic_management') {
                        return !isGuest && user?.role === 'clinic';
                      }
                      return true;
                    })
                    .map(item => {
                  ...
                  <div className="border-t border-[#e3dec9] my-2 pt-3">
                    ...
                    <motion.button
                      ...
                      onClick={() => {
                        setIsSidebarOpen(false);
                        if (isGuest) {
                          if (onRequireAuth) onRequireAuth("Please log in or sign up to message clinics and veterinarians.");
                          return;
                        }
              ...
              {/* Bottom Region (Profile & Signout or Visitor Login Card) */}
              {isGuest ? (
                <div className="p-5 border-t border-[#f4f1e9] bg-[#fcf9f2]/90 flex flex-col gap-3">
                  <p className="text-[10px] uppercase font-bold text-[#a49f92] tracking-wider">Visitor Session</p>
                  <div className="p-3.5 rounded-2xl border border-[#e3dec9] bg-white flex items-center gap-3 shadow-2xs">
                    <div className="w-10 h-10 rounded-full bg-[#5a5a40] text-white flex items-center justify-center font-bold text-sm shadow-xs">
                      🐾
                    </div>
                    <div className="flex flex-col">
                      <span className="font-bold text-sm leading-tight text-[#3c3c3b]">Guest Visitor</span>
                  ...
                  <button
                    onClick={() => {
                      setIsSidebarOpen(false);
                      if (onRequireAuth) onRequireAuth("Log in or create a free account to unlock verified messaging, job applications, and clinic management.");
                    }}
                    className="w-full flex items-center justify-center gap-2 px-4 py-3.5 bg-[#5a5a40] text-white hover:bg-[#3e3e2b] border border-[#3e3e2b] border-b-[4px] border-b-[#2e2e1f] rounded-xl text-xs font-black transition-all cursor-pointer shadow-sm"
